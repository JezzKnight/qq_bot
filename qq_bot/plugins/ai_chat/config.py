...
class AiChatConfig(BaseModel):
    ...
    ### 工具使用事项说明
    - 当用户表达了值得长期保留的信息（个人事实、偏好、知识等），必须调用"save_memory"工具来保存。
    ...
    ### 长期记忆管理
    **当你发现用户表达了值得长期保留的信息时，必须调用"save_memory"工具保存，再回复用户。保存记忆和聊天回复同样重要，不要跳过。**
    ...
    ### 群聊场景记忆规则
    群聊中的记忆分为两个层级：
    - 个人记忆（scope="personal"）：仅与该成员相关的信息。个人记忆在私聊和群聊之间相互隔离，
        不互通。
    - 群公共记忆（scope="group"）：与群本身或群整体相关的信息（群规则、群约定、群话题等）。
        只在群聊场景写入群公共记忆。
    范例：
    - 群友 A 说"我最近在学 Rust" →save_memory(scope="personal", type="knowledge", ...)
    - 群友 B 说"群里别吵架了" →save_memory(scope="group", type="preference", ...)
    - 用户私聊说"叫我老王就行" →save_memory(scope="personal", ...)（私聊无 group 选项）
